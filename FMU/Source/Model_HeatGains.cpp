 ...
 */
void Model_HeatGains::aHCTCLcalc(
        double ta,
        double icl,
    ...
    double p3 = p1 * 100;
    double p4 = p1 * ata;
    double p5 = 308.7 - 0.028 * (metabolicRate - externalWork) +
                                p2 * pow(actualMeanRadiantTemperature / 100, 4);
    xn = stcl / 100;
    double xf = xn / 2;
...
void Model_HeatGains::calculate(
    ...
    // double partialWaterPressure =
    //      computePaCIBSEGuideC(actualMeanRadiantTemperature, reativeHumidity);
    double partialWaterPressure = reativeHumidity * 10 *
                                          exp(16.6536 - 4030.183 / (ta + 235));
    ...
    aHCTCLcalc(ta, icl, airVelocity, surfaceAreaOfClothing,
                metabolicRate, externalWork, actualMeanRadiantTemperature);
    double TS = 0.303 * exp(-0.036 * metabolicRate) + 0.028;
    // vapourDiffusion + sweatEvaporation + latentRespirationHeatGains
    // = Vapour diffusion + sweat evaporation
    // + latent respiration heat gains (all the latent stuff)
    vapourDiffusion = (3.05 * pow(10, -3)) * (5733 - 6.99 *
                        (metabolicRate - externalWork) - partialWaterPressure);
    sweatEvaporation = 0;
    if ((metabolicRate - externalWork) > 58.15) {
        sweatEvaporation = 0.42 * (metabolicRate - externalWork - 58.15);
    }
    latentRespirationHeatGains = 1.7 * 0.00001 * metabolicRate *
                                                (5867 - partialWaterPressure);
    // dryRespiration + convectiveHeatGains
    // = Dry respiration + convective heat gains
    // (all the sensible convective stuff)
    dryRespiration = 0.0014 * metabolicRate * (34 - ta);
    radiantHeatGains = 3.96 * surfaceAreaOfClothing * (pow(xn, 4)
            - pow(actualMeanRadiantTemperature / 100, 4));
    convectiveHeatGains = surfaceAreaOfClothing * hc * (tcl - ta);
    pmv = TS * (metabolicRate - externalWork
            - vapourDiffusion - sweatEvaporation - latentRespirationHeatGains
            - dryRespiration - radiantHeatGains - convectiveHeatGains);
 ...
 */
double Model_HeatGains::computePaCIBSEGuideC(
            double actualMeanRadiantTemperature, double reativeHumidity) const {
    double Ps = pow(10, (30.59051 - 8.2 * log10(actualMeanRadiantTemperature)
                  + 2.4804 * pow(10.0, -3.0) * actualMeanRadiantTemperature
                  - (3142.31 / actualMeanRadiantTemperature)));
