...
Agent_Action_Appliance_BDI::Agent_Action_Appliance_BDI() {
  ApplianceDuringDay = 0.0;
}

void Agent_Action_Appliance_BDI::setApplianceDuringDay(
                                      double ApplianceDuringDay) {
  this->ApplianceDuringDay = ApplianceDuringDay;
}

bool Agent_Action_Appliance_BDI::doRecipe(
...
void Agent_Action_Appliance_BDI::calculateFirstLastCouts(
      const std::vector<double> &activities) {
  int stepCount = SimulationConfig::getStepCount();
  if (stepCount > first) {
