...
TEST_F(Test_Appliance_Usage, on) {
  ...
  for (int app = 0; app < 6; app++) {
    ...
    mau.setID(app);
    mau.parseConfiguration(testFiles + "AppliancesLarge.xml");
...
TEST_F(Test_Appliance_Usage, on_with_varied_timestep) {
  ...
  for (int app = 0; app < 6; app++) {
    ...
    for (int i = 0; i < sims; i++) {
      for (int timeStep = 0; timeStep < 1440; timeStep++) {
          int x = (timeStep / 10);
        on[x] += mau.onAt(timeStep);
      }
    }
    std::string file = "";
    switch (app) {
    ...
    for (int timeStep = 0; timeStep < 144; timeStep++) {
      switch (app) {
        case 3:
          ...
          break;
        case 2:
            EXPECT_NEAR(on[timeStep] / sims, onMicroWave[timeStep], 0.005);
          break;
        case 0:
            EXPECT_NEAR(on[timeStep] / sims, onTv[timeStep], 0.005);
          break;
...
TEST_F(Test_Appliance_Usage, totalEnergy) {
  ...
  for (int app = 0; app < 6; app++) {
    ...
    for (int i = 0; i < sims; i++) {
      for (int timeStep = 0; timeStep < 144; timeStep++) {
        consumption[timeStep] += mau.consumption(timeStep);
      }
    }
...
TEST_F(Test_Appliance_Usage, meanFraction) {
  ...
  for (int app = 0; app < 6; app++) {
    ...
    for (int i = 0; i < sims; i++) {
      for (int timeStep = 0; timeStep < 144; timeStep++) {
        if (mau.onAt(timeStep)) {
          consumption[timeStep] += mau.getMeanFraction();
        }
      }
    }
